...
$fetch_info ="http://ign-apis.herokuapp.com/videos?startIndex=30&count=5";
...
//gathers and stores data
for($i=0; $i<5; $i++){
	$tempLink="";
	$arrayBG[$i] = "url(" . $jsondecoded['data'][$i]["thumbnail"].")";
	$arrayHead[$i] = $jsondecoded['data'][$i]['metadata']["name"];
	if(isset($jsondecoded['data'][$i]['metadata']["description"])){
		$arraySubHead[$i] = $jsondecoded['data'][$i]['metadata']["description"];
		$durationArray[$i] = gmdate("i:s",$jsondecoded['data'][$i]['metadata']["duration"]);
	...
	//echo $dateArray[1];
	//uses the date and slug to make igns standard link
	for($j=0; $j< count($dateArray);$j++){
		...
		$newLink="http://www.ign.com/videos/".$newLink."/".$slugArray[$j];
		$linkArray[$j]=$newLink;
		//echo $newLink;
		//echo $linkArray[$j];
		$newLink="";
	...
	$shortenHead="";
	$shortenSub="";
	//used to shorten the length of arrays that are too long and add "..."
	...
	//used to remove leading 0s in duration
	for($i=0;$i<count($durationArray);$i++){
		for($j=0;$j<strlen($durationArray[$i]);$j++){
			if($j==0 && $durationArray[$i][$j]=="0" ){
				
			}
			else{
				$tempDur=$tempDur.$durationArray[$i][$j];
			}
		}
		$durationArray[$i]=$tempDur;
		$tempDur="";
	}
...
function addListeners() {
	...
	document.getElementById("p0D").innerHTML = "<?php echo $durationArray[0]?>";
	...
	document.getElementById("p1D").innerHTML = "<?php echo $durationArray[1]?>";
	...
	document.getElementById("p2D").innerHTML = "<?php echo $durationArray[2]?>";
	...
	document.getElementById("p3D").innerHTML = "<?php echo $durationArray[3]?>";
	...
	document.getElementById("p4D").innerHTML = "<?php echo $durationArray[4]?>";
